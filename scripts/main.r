...
cat("Regresión lineal de Y sobre X E(Y|X):\n")
# v.a X
...
r <- xy_cov / (x_sd * y_sd)
cat(sprintf("Coeficiente de correlación muestral: %s\n", r))

#* N°2 Determinar coeficientes de la recta de regresión lineal
#* E(Y|X) = aX + b
a <- xy_cov / x_var
b <- y_avg - a * x_avg
...
cat("¿Es útil conocer el valor de X para estimar E(Y|X)?:\n")
cat(" Como el coeficiente de correlación es cercano a 1 esto sugiere 
una relación lineal positiva entre las dos variables. Esto significa que,
